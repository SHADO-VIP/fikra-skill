...
Treat «عربية مبسطة»، «عربية واضحة»، «عربية سهلة» and «عربية مهنية وقريبة» as simplified Modern Standard Arabic unless the user explicitly requests a specific dialect. A friendly or approachable tone does not authorize colloquial language. Apply this to final on-image/on-screen text and captions as well as explanations; follow the register review in the Arabic-style reference. If explicitly requested dialect quality is uncertain, use natural Modern Standard Arabic rather than inventing expressions.
...
Read [references/safety-and-credibility.md](references/safety-and-credibility.md) whenever the idea or drafted copy includes:

- Statistics, studies, sources, or performance claims
- Behavioral outcomes or claims that a duration/action is sufficient to form a habit, even without statistics; distinguish these from simple invitations to try an activity
