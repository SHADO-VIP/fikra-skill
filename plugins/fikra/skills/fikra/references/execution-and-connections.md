...
3. For images/videos, complete the visual-text approval gate below first. Then present the suitable tools, expected files and relevant known costs/credits only. If unknown, state that briefly without estimates or unsupported credit warnings. Apply the tool-choice gate below; obtain approval for each named tool or clearly bounded group of operations before use.
...
After choosing Instagram slides and explicitly approving their exact text, only if tool discovery confirms the capability and potential credit usage:
...
## Mandatory Visual-Text Review and Approval

Before producing or editing any image or video, keep three decisions distinct and ordered: approval of the campaign idea/angle, approval of the final text inside the selected assets, then separate permission to use the production tool. Angle approval never approves slide/scene copy automatically; tool permission never approves text. Do not combine text approval and tool permission into one question or infer either from the other.

1. Review the exact visible text using the meaning-clarity criteria in [arabic-style.md](arabic-style.md#meaning-clarity-in-visual-copy). Present the [visual brief’s copy sheet](campaign-output.md#8-visual-brief) in clear language for every selected asset, including every slide/scene and all text that will appear. Briefly explain the intended message of each, especially where interpretation could vary. For an asset with no visible text, state that explicitly for approval instead of inventing copy.
2. Ask explicitly: «هل تعتمد النص النهائي لكل شريحة أو مشهد كما عرضته، أم تريد تعديله؟» Wait for the answer; a draft, silence, or campaign-angle approval is insufficient. Record the approved asset identifiers and exact text versions. Honor an existing explicit approval of that exact text; changed text requires renewed text approval.
3. Only after text approval, obtain separate tool permission under the gate below, tied to those approved versions. Do not call image/video production or editing tools until both gates are satisfied. Disclosed batches must identify the text versions of every included asset.

If the user says the text is unclear, stop production. Explain the intended message briefly, propose clearer copy, and ask for its approval. Mark any existing image/video carrying the unclear text **«تحتاج إلى مراجعة بسبب عدم وضوح النص»**; it is not a final asset even if its Arabic rendering, spelling and visual design passed inspection. Preserve that file and its original production prompt. Record the previous text, replacement version and approval evidence, and explicitly state when the existing media still displays the previous version and does not match the newly approved copy.

After this correction, regeneration or editing requires **new explicit tool permission after the corrected text is approved**, even if the same tool or an earlier retry was authorized. Text approval alone permits no media call. Keep production paused when the user requests only a text/status correction. Inspect any later authorized output against the approved copy and its intended meaning as well as visual/technical quality before calling it final.
...
- `execution-status.md`: selected route and asset subset, completed text versus pending media, deferred/excluded assets, manual-delivery choice, separate idea/angle, exact text-version and tool approvals, text-to-media version correspondence, stage history and next decision; update after every stage or user change. Track each requested output, current status (`مكتمل`, `بانتظار موافقة`, `يحتاج إلى أداة`, `يحتاج إلى ربط`, `فشل`, `نجاح جزئي`, `لم يبدأ`, `تحتاج إلى مراجعة`), actual file path if it exists, tool/provenance when useful, verification evidence/limitations, blocker, and next action. Include the overall operating state and resumable checkpoint. If the deliverable list is missing, mark its extent as unknown instead of inventing counts or platforms.
