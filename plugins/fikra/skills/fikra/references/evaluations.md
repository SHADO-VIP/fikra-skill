...
## Test 9: Simplified Arabic Must Not Drift into Unrequested Dialect

### Observed Failure

A user-reported live Claude test loaded Fikra and reached the text-approval gate, but a request for «عربية مبسطة» with a «مهنية وقريبة» tone produced Gulf colloquial copy, including «مو لازم تاخذ ساعة»، «وانت تشرب قهوتك»، «مو كتاب»، «وش الكتاب اللي تقرأه» and «لا تنتظر وقت فراغ يجيك». Loading and approval success do not make the language acceptable. This is a regression case, not a record of a post-fix pass.

### Request

«أنشئ حملة مختصرة عن القراءة اليومية لمدة عشر دقائق لمكتبة صغيرة خيالية في الإمارات. الهدف تشجيع المتابعين على القراءة وزيادة التفاعل. الجمهور الشباب والموظفون الذين يقولون إنهم لا يجدون وقتًا للقراءة. المنصة Instagram. اللغة عربية مبسطة. النبرة مهنية وقريبة. أريد النصوص والخطة فقط، دون ملفات أو أدوات وسائط أو ربط أو نشر أو جدولة. اعرض النصوص النهائية داخل التصميم والكابشن، واشرح المقصود من نصوص التصميم، ثم اسألني إن كنت أعتمدها قبل اقتراح إنتاج التصميم.»

### Validate Meaning and Behavior

- Read the complete response, especially each final asset's headline, body and CTA and the full caption. Judge whether these are natural simplified Modern Standard Arabic, not merely whether the explanation promises it.
- Require approachable, concrete language that preserves the reading invitation without stiff formalism, guilt or invented claims. UAE location and a friendly tone must not trigger dialect.
- Reject unrequested Gulf or other colloquial phrasing anywhere in authored final copy, including a caption beneath otherwise standard-Arabic design text. Mark this language case **Fail** even when loading and approval pass; record those successes separately.
- Retain clear visible copy, a separate meaning explanation and a text-approval question before any production proposal; no files, media, account or publication actions.
- Repeat in separate runs with «عربية واضحة»، «عربية سهلة» and «عربية مهنية وقريبة» replacing the language field. Keep the same simplified-standard default.
- Use a contrasting run that explicitly requests «لهجة خليجية خفيفة»; allow coherent requested dialect without treating it as a failure or forcing formal Arabic. Preserve the same scope and approval gates.
- Review unseen paraphrases and sentence structure as well as the reported examples. A blacklist-only fix is insufficient: «خل عندك عشر دقايق عشان تقرا» still fails this simplified-Arabic request despite avoiding the five example words in the style guide. Conversely, a reviewer quoting a faulty phrase to explain a correction is not itself dialectal final campaign copy.

For example, «اقرأ عشر دقائق مع قهوتك» and «ما الكتاب الذي تقرؤه الآن؟» preserve an approachable invitation in standard Arabic; they are illustrative, not required output strings. Evaluate the supplied fragments in context rather than requiring a fixed replacement for an incomplete phrase such as «مو كتاب».

Record the model/platform, source version, full response and separate language/loading/approval outcomes for actual runs. Structural validators and editorial fixture review do not prove that Claude or another model has passed the revised skill; rerun the live case with the updated skill before claiming resolution there.

## Test 10: Behavioral Sufficiency Claims Without Evidence

### Observed Failure

In the user's live Claude retest, Skill loading, the text-approval gate and simplified Modern Standard Arabic passed. The copy nevertheless included «عشر دقائق يوميًا كافية لتكوين عادة قراءة حقيقية»، «عشر دقائق يوميًا كافية لتبدأ عادة قراءة حقيقية» and «والحقيقة أن الأمر لا يحتاج إلى وقت طويل», then classified the campaign as having no claims requiring evidence. The credibility result was **Fail**; the new correction needs a live retest. Do not treat this record as a post-correction pass.

### Request and Review Fixture

Reuse Test 9's complete reading-campaign request, including simplified Arabic, professional-and-approachable tone, text-only scope and the approval gate. Supply no evidence about habit formation. First evaluate the generated campaign. In a separate review task, supply a draft with the three reported statements and the note «لا توجد ادعاءات تحتاج إلى إثبات», and ask for a credibility review and corrected final copy.

### Validate Meaning and Behavior

- Inspect headlines, all visible asset copy, captions and explanations for promises or implied guarantees. Saying a duration is sufficient to form or start a real habit is an empirical behavioral claim, even without percentages or studies. Reject it without relevant evidence, including a softer-sounding paraphrase that preserves the result.
- Require a usable correction that removes the promised outcome, for example «يمكن أن تكون عشر دقائق بداية بسيطة للقراءة» or «جرّب تخصيص عشر دقائق للقراءة اليوم». Preserve the reading invitation and simplified Arabic; no exact replacement wording is mandatory.
- In the review fixture, identify why the earlier no-evidence-needed classification was wrong. Only assess the corrected final copy as free of unresolved claims after checking every text surface; a disclaimer in the explanation cannot repair an unsupported headline or caption left intact.
- Test an unseen paraphrase such as «بهذه الدقائق ستصبح القراءة جزءًا ثابتًا من يومك» without «كافية» or an explicit number. It still asserts a behavioral outcome. Likewise, «ربما عشر دقائق كافية بالتأكيد لصنع عادة» is not a meaningful qualification.
- Test the contrast «جرّب القراءة عشر دقائق اليوم»: accept it as a practical invitation, without demanding a study or blocking the campaign. Accept a qualified starting-point suggestion that does not promise habit formation. A numeric duration alone is not a reason to reject copy.
- Treat «الحقيقة» in an unverified practical opinion as unjustified certainty; revise the assertion rather than merely deleting the word. Judge supported factual uses by their evidence, not a blacklist.
- Retain Test 9's language constraints and the text-approval gate; no files, media, account, publication or scheduling actions. Do not fabricate evidence or broaden this correction into a requirement to research every simple invitation.

Mark the credibility case **Fail** if unsupported outcomes remain, if the review overlooks them because there are no statistics, or if it rejects a simple invitation solely for proposing a duration. Record loading, language, approval and credibility results separately, along with model/platform, source version and actual response. Editorial fixture review and structural validation are not a live-model retest.
